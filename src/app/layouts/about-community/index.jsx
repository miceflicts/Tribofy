...
import { Star, LoaderCircle, Loader } from "lucide-react";

import { Rating } from "react-simple-star-rating";
import { Button } from "@/components/ui/button";

import { ShieldCheck } from "lucide-react";
import JoinCommunityModal from "@/app/components/about-community/joinCommunity";
...
export default function AboutCommunityPage({
  ...
  useEffect(() => {
    if (
      isInCommunity.isInCommunity !== undefined &&
  ...
  return (
    <>
      <div className="mt-14 flex h-[70vh] w-[90vw] gap-14 rounded-xl border border-border bg-card p-10">
        {communityData?._id !== undefined && userData?.user !== undefined && (
          <>
            {isJoiningCommunity && (
              <JoinCommunityModal
            ...
            <div className="flex h-fit w-[37%] flex-col gap-6">
              ...
              <div className="text-normal flex h-fit w-full font-light text-text-default/80">
                <span className="">
                  Join the #1 Clothing Brand Community in the WORLD. WHATEVER
                  PRICE YOU JOIN AT IS WHAT YOU GET FOREVER. Learn Everything I
                  Know as a 7 Figure Brand Owner at 18 What you will get: ​Elite
                  Network of Brand Owners and Online Bread Winners Live Weekly
                  Support Calls​ Access to Design Coaches Access to Paid Ads
                  Coaches ​Special Guest Masterclasses ​Full A-Z Clothing Brand
                  Guide ​Full A-Z Marketing Guide ​Full A-Z Social Media Guide
                  ​My Personal Manufactures and Designers List ​Credit and Money
                  Management Guide ​Full Resellers Guide Ranking/Rewards System
                  ​ 24/7 Support
                </span>
              </div>
            </div>
            ...
            <div className="flex h-full w-[20%]">
              <div className="flex h-fit w-full flex-col gap-6 rounded-xl border border-border p-5">
                ...
                {canJoinCommunity ? (
                  <Button
                    className="w-full bg-button py-6 text-base text-button-text hover:bg-button-hover active:bg-button-active"
                  ...
                  <Button className="w-full bg-button py-6 text-base text-button-text hover:bg-button-hover active:bg-button-active">
                    <LoaderCircle
                      size={20}
                      className="animate-spin text-text-default"
                    ></LoaderCircle>
                  </Button>
                )}
